...
int uname(struct utsname *n) { DWORD size=sizeof(n->nodename); SYSTEM_INFO i; const char *machine; strcpy_s(n->sysname,sizeof(n->sysname),"Windows");GetComputerNameA(n->nodename,&size);GetNativeSystemInfo(&i);switch(i.wProcessorArchitecture){case PROCESSOR_ARCHITECTURE_AMD64:machine="x86_64";break;case PROCESSOR_ARCHITECTURE_ARM64:machine="aarch64";break;default:machine="unknown";break;}strcpy_s(n->machine,sizeof(n->machine),machine);strcpy_s(n->release,sizeof(n->release),"NT");strcpy_s(n->version,sizeof(n->version),"native");return 0; }
